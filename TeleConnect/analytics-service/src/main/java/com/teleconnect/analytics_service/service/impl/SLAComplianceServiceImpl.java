...
public class SLAComplianceServiceImpl implements SLAComplianceService {
    ...
    private static final Logger log = LoggerFactory.getLogger(SLAComplianceServiceImpl.class);

    private final FaultTicketRepository faultTicketRepository;

    public SLAComplianceServiceImpl(FaultTicketRepository faultTicketRepository) {
    ...
    public SLAComplianceResponse computeSLACompliance(LocalDate periodStart, LocalDate periodEnd) {
        List<FaultTicket> closedTickets = faultTicketRepository.findClosedInPeriod(
        ...
        for (FaultTicket ticket : closedTickets) {
        ...
        long escalatedCount = faultTicketRepository
                .findByStatusIn(List.of(FaultStatus.ESCALATED)).size();
    ...
    public void escalateBreachingTickets() {
        ...
        for (FaultPriority priority : FaultPriority.values()) {
            ...
            List<FaultTicket> breaching = faultTicketRepository.findOpenBreachingBefore(slaDeadline)
                    .stream()
                    .filter(t -> t.getPriority() == priority)
                    .toList();

            for (FaultTicket ticket : breaching) {
                ticket.setStatus(FaultStatus.ESCALATED);
                faultTicketRepository.save(ticket);
                log.warn("Escalated ticket {} (Priority: {}) - breached SLA of {}h",
