...
public class ExportController {
    ...
    public ResponseEntity<byte[]> exportNetworkUtilisation(
        ...
        try {
            if ("csv".equalsIgnoreCase(format)) {
                return csvResponse(exportService.exportNetworkUtilisationAsCsv(data), "network-utilisation-cycle-" +
                        cycleId + ".csv");
            }
            return pdfResponse(exportService.exportNetworkUtilisationAsPdf(data), "network-utilisation-cycle-" +
                    cycleId + ".pdf");
        } catch (IOException e) {
            throw new AnalyticsException("Export failed: " + e.getMessage());
        }
    ...
    public ResponseEntity<byte[]> exportCollectionEfficiency(
        ...
        try {
            if ("csv".equalsIgnoreCase(format)) {
                return csvResponse(exportService.exportCollectionEfficiencyAsCsv(data),
                        "collection-efficiency-cycle-" + cycleId + ".csv");
            }
            return pdfResponse(exportService.exportCollectionEfficiencyAsPdf(data), "collection-efficiency-cycle-" +
                    cycleId + ".pdf");
        } catch (IOException e) {
            throw new AnalyticsException("Export failed: " + e.getMessage());
        }
